...
md"""
In our study of the role of the atmosphere on Earth’s energy budget we only considered the lower layer of the atmosphere, the troposphere, which contains most of the atmospheric mass and dominates the absorption/emission of thermal/longwave radiation. In this problem, you will consider the role played by the stratosphere, the
atmospheric layer above the stratosphere. Observations suggest that the stratosphere has been cooling over the last century, a trend opposite to what has been well documented in the troposphere. The stratosphere is rich in ozone, a gas that absorbs shortwave radiation. Thus, unlike the troposphere, whose main source of radiative forcing is longwave radiation from the surface, the stratosphere is also heated by absorbed shortwave radiation. This leads to a different stratospheric response to an increase in CO``_2`` concentrations.

To represent the response of the stratosphere to an increase in CO``_2`` concentrations, add a third layer to the Earth energy balance model composed of an ocean and a troposphere, such that ``T_o`` is the ocean temperature, ``T_a`` the tropospheric temperature, and ``T_{st}`` the newly added stratospheric temperature. Assume that a fraction ``β_{tr}`` of the shortwave solar radiation is absorbed by the stratosphere. energy balance for the ocean, troposphere and stratosphere. 
"""

# ╔═╡ db915c60-d201-4855-bc1e-b5fd7405d43b
